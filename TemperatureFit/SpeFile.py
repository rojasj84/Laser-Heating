...
class SpeFile(object):
    ...
    def _read_arbitrary(self):
        """Reads center wavelength position from the header into the center_wavelength field"""
        self.read_arbitrary = float(self._read_at(1510, 1, np.int16)[0])
        self.read_arbitrary2 = float(self._read_at(3183, 1, np.float64)[0])
